...
//Add Cat
export const addCats = (name) => async (dispatch) => {
  try {
    dispatch({
...
export const updateCat = (cat) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `/api/products/updateCat/${cat._id}`,
      cat,
      config
    );
...
export const deleteCat = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`/api/products/deleteCat/${id}`, config);
  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `/api/products/getCats?keyword=${keyword}&pageNumber=${pageNumber}`
      );
